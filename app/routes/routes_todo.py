...
async def crear_todo_endpoint(
    ...
    return await crud_todo.crear_todo(db=db, todo=todo, usuario_id=usuario_actual.id)
...
async def eliminar_todos_completados_endpoint(
    ...
    num_eliminados = await crud_todo.eliminar_todos_completados(db=db, usuario_id=usuario_actual.id)
...
async def obtener_todo_endpoint(
    ...
    db_todo = await crud_todo.obtener_todo(db=db, todo_id=todo_id, usuario_id=usuario_actual.id)
    if db_todo is None:
...
async def obtener_todos_endpoint(
    ...
    todos = await crud_todo.obtener_todos(db=db, usuario_id=usuario_actual.id, skip=skip, limit=limit)
    return todos
...
async def actualizar_todo_endpoint(
    ...
    db_todo = await crud_todo.actualizar_todo(db=db, todo_id=todo_id, todo_update=todo_update, usuario_id=usuario_actual.id)
    if db_todo is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Todo no encontrado")
    return db_todo
...
async def eliminar_todo_endpoint(
    ...
    db_todo = await crud_todo.eliminar_todo(db=db, todo_id=todo_id, usuario_id=usuario_actual.id)
    if db_todo is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Todo no encontrado")
...
async def obtener_todos_por_prioridad_igual_endpoint(
    ...
    todos = await crud_todo.obtener_todos_por_prioridad_igual(
...
async def obtener_todos_por_prioridad_mayor_igual_endpoint(
    ...
    todos = await crud_todo.obtener_todos_por_prioridad_mayor_igual(
        db=db, usuario_id=usuario_actual.id, prioridad=prioridad.value, skip=skip, limit=limit
    )
    return todos
...
async def obtener_todos_por_prioridad_menor_igual_endpoint(
    ...
    todos = await crud_todo.obtener_todos_por_prioridad_menor_igual(
        db=db, usuario_id=usuario_actual.id, prioridad=prioridad.value, skip=skip, limit=limit
    )
    return todos
